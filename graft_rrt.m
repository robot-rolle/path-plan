...
map = imread('../RRT_START/rrt*n5.bmp');
    source=[1 1]; 
...
while  countt>0,
    ...
    global_length = 0;
    local_length = 0;
    RRTree_main=double([source -1 ]);
    RRTree_goal=double([goal -1 ]);
...
tic;
    while  ~donytic_param_data(3) || ~donytic_param_data(4) ,
        ...
        if ~donytic_param_data(1) && ~donytic_param_data(3),
            [RRTree_main,static_param_data,donytic_param_data]=g_rrta(RRTree_main,GraftTree,...
        ...
        if  ~donytic_param_data(2) && ~donytic_param_data(4),
            [RRTree_goal,static_param_data,donytic_param_data]=g_rrtb(RRTree_main,...
    ...
    for i=1:length(in_path)-1, pathLength=pathLength+distanceCost(in_path(i,1:2),in_path(i+1,1:2)); end
